  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div id="safety" className="mt-16 grid grid-cols-1 sm:grid-cols-2 gap-4">
        {CALLOUTS.map((c) => (
          <div key={c.title} className="glass rounded-xl p-5 flex flex-col gap-2">
            <span className="font-mono text-xs font-semibold text-white">{c.title}</span>
            <p className="text-[12px] text-slate-400 leading-relaxed flex-1">{c.body}</p>
            <Link
              href={c.href}
      ...
      <div className="mt-20 max-w-3xl space-y-12">
        <section>
          <h2 className="text-xl font-bold text-white mb-3">What is the Claude Execution Lab?</h2>
          ...
          <p className="text-slate-500 leading-relaxed text-sm mt-3">
        ...
        <section>
          <h2 className="text-xl font-bold text-white mb-3">Claude Code workflow simulation</h2>
          <p className="text-slate-400 leading-relaxed text-sm">
          ...
          <h3 className="text-sm font-semibold text-slate-300 mt-4 mb-2">Key phases</h3>
        ...
        <section>
          <h2 className="text-xl font-bold text-white mb-3">MCP workflow simulation</h2>
          <p className="text-slate-400 leading-relaxed text-sm">
          ...
          <h3 className="text-sm font-semibold text-slate-300 mt-4 mb-2">About MCP</h3>
        ...
        <section>
          <h2 className="text-xl font-bold text-white mb-3">Agent Skills workflow simulation</h2>
          <p className="text-slate-400 leading-relaxed text-sm">
            The Agent Skills simulation shows how Claude Code might invoke the <code className="text-slate-300 font-mono text-xs px-1 py-0.5 rounded bg-white/5">/pr-describe</code> skill — scanning the skills directory, loading the SKILL.md instructions, running an allowed helper script against the git diff, and generating a structured PR description.
          </p>
          <h3 className="text-sm font-semibold text-slate-300 mt-4 mb-2">About Agent Skills</h3>
          <p className="text-slate-500 text-sm">
            An Agent Skill is a directory containing a <code className="text-slate-300 font-mono text-xs px-1 py-0.5 rounded bg-white/5">SKILL.md</code> file with YAML frontmatter specifying the skill name, description, and allowed tools.
            Skills are invocable as slash commands in Claude Code and are supported in the Claude Agent SDK. They are loaded from <code className="text-slate-300 font-mono text-xs px-1 py-0.5 rounded bg-white/5">.claude/skills/</code> in the project workspace or installed from the official <code className="text-slate-300 font-mono text-xs px-1 py-0.5 rounded bg-white/5">anthropics/skills</code> repository.
            Behavior may vary by Claude surface and configuration.
          </p>
        </section>

        <section>
          <h2 className="text-xl font-bold text-white mb-3">Why human review still matters</h2>
          <p className="text-slate-400 leading-relaxed text-sm">
        ...
        <section>
          <h2 className="text-xl font-bold text-white mb-3">Related Eggthropic labs</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
            {[
              ...
              <Link
                key={link.href}
                href={link.href}
                className="glass rounded-xl p-4 hover:border-white/16 border border-white/8 transition-all group"
              >
                <span className="text-sm font-semibold text-slate-200 group-hover:text-white transition-colors block mb-1">
                  {link.label}
                </span>
        ...
        <section>
          ...
          <div className="space-y-5">
            {FAQ_ITEMS.map(({ q, a }) => (
              <div key={q} className="border-l-2 border-white/10 pl-4">
                <h3 className="text-sm font-semibold text-slate-200 mb-1.5">{q}</h3>
