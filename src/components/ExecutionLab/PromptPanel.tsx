...
export default function PromptPanel({ scenario, status, onRun, onPause, onResume, onReset }: Props) {
  ...
  return (
    <div className="glass rounded-xl p-4 flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2 pt-1">
        ...
        {/* Reset (any non-idle state) */}
        {!isIdle && (
          <motion.button
            initial={{ opacity: 0, y: -4 }}
            animate={{ opacity: 1, y: 0 }}
            onClick={onReset}
            className="w-full px-4 py-2 rounded-lg text-xs font-mono text-slate-500 hover:text-slate-300 glass border border-white/8 hover:border-white/14 transition-all active:scale-95"
          >
            ↺ Reset
          </motion.button>
        )}
